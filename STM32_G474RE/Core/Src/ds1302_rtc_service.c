/**
 ******************************************************************************
 * @file    ds1302_rtc_service.c
 * @author  Jack Lestrohan
 * @brief   RTC Module service file
 ******************************************************************************
 */
...
#define DS1302_SCLK	DS1302RTC_CLK_Pin
#define DS1302_SDA	DS1302RTC_SDA_Pin
#define DS1302_RST	DS1302RTC_RST_Pin
...
static void DS1302_SendCmd(uint8_t cmd)
{
	...
	for (i = 0; i < 8; i++) {
		...
		HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SDA,
		        (cmd & 1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
		/*		DS1302_SCK = 1; */
...
static void DS1302_WriteByte(uint8_t addr, uint8_t d)
{
	...
	for (i = 0; i < 8; i++) {
		/*		DS1302_SDA = (bit)(d & 1); */
		HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SDA,
		        (d & 1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
		/*		DS1302_SCK = 1; */
		HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SCLK, GPIO_PIN_SET);
		delayUS_DWT(1);
		/*		DS1302_SCK = 0; */
...
static void DS1302_WriteBurst(uint8_t cmd, uint8_t len, uint8_t *temp)
{
	...
	for (j = 0; j < len; j++) {
		for (i = 0; i < 8; i++) {
			/*			DS1302_SDA = (bit)(d & 1); */
			HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SDA,
			        (temp[j] & 1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
			/*			DS1302_SCK = 1; */
			HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SCLK, GPIO_PIN_SET);
			delayUS_DWT(1);
			/*			DS1302_SCK = 0; */
...
static uint8_t DS1302_ReadByte(uint8_t addr)
{
	...
	for (i = 0; i < 8; i++) {
		temp >>= 1;
		/*		if(DS1302_SDA) */
		if (HAL_GPIO_ReadPin(DS1302_GPIO, DS1302_SDA)) temp |= 0x80;
		/*		DS1302_SCK = 1; */
		HAL_GPIO_WritePin(DS1302_GPIO, DS1302_SCLK, GPIO_PIN_SET);
		delayUS_DWT(1);
		/*		DS1302_SCK = 0; */
	...
	for (j = 0; j < len; j++) {
		*(temp + j) = 0;
		for (i = 0; i < 8; i++) {
			*(temp + j) >>= 1;
			/*			if(DS1302_SDA) */
			if (HAL_GPIO_ReadPin(DS1302_GPIO, DS1302_SDA)) *(temp + j) |= 0x80;
