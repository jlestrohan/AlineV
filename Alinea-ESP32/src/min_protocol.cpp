...
enum
{
    HEADER_BYTE = 0xaaU, //!< HEADER_BYTE
    STUFF_BYTE = 0x55U,  //!< STUFF_BYTE
    EOF_BYTE = 0x55U,    //!< EOF_BYTE
};
...
/**
 * @brief Pops frame from front of queue, reclaims its ring buffer space
 * @param self
 */
...
/**
 * @brief Claim a buffer slot from the FIFO. Returns 0 if there is no space.
 * @param self
 * @param data_size
 * @return
 */
...
/**
 * @brief Return the nth frame in the FIFO
 * @param self
 * @param n
 * @return
 */
...
/**
 * @brief Sends the given frame to the serial line
 * @param self
 * @param frame
 */
...
/**
 * @brief We don't queue an ACK frame - we send it straight away (if there's space to do so)
 * @param self
 */
...
/**
 * @brief We don't queue an RESET frame - we send it straight away (if there's space to do so)
 * @param self
 */
static void send_reset(struct min_context *self)
{
...
/**
 * @brief 	Queues a MIN ID / payload frame into the outgoing FIFO
 * 			API call.
...
/**
 * @brief Finds the frame in the window that was sent least recently
 * @param self
 * @return
 */
...
/**
 * @brief 	This runs the receiving half of the transport protocol, acknowledging frames received, discarding
...
/**
 * @brief API call: sends received bytes into a MIN context and runs the transport timeouts
 * @param self
 * @param buf
 * @param buf_len
 */
...
/**
 * @brief Sends an application MIN frame on the wire (do not put into the transport queue)
 * @param self
 * @param min_id
 * @param payload
 * @param payload_len
