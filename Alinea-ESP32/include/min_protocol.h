...
/**
 * @brief Send a non-transport frame MIN frame
 * @param self
 * @param min_id
 * @param payload
 * @param payload_len
...
/**
 * @brief	Must be regularly called, with the received bytes since the last call.
...
/**
 * @brief Reset the state machine and (optionally) tell the other side that we have done so
 * @param self
 * @param inform_other_side
 */
...
/**
 * @brief 	CALLBACK. Must return current time in milliseconds.
...
/**
 * @brief CALLBACK. Must return current buffer space in the given port. Used to check that a frame can be queued
 * @param port
 * @return
 */
...
/**
 * @brief CALLBACK. Send a byte on the given line.
 * @param port
 * @param byte
 */
...
/**
 * @brief CALLBACK. Indcates when frame transmission is finished; useful for buffering bytes into a single serial call.
 * @param port
 */
...
/**
 * @brief Initialize a MIN context ready for receiving bytes from a serial link
